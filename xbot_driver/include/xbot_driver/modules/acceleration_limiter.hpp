...
public:
   ...
   */
  std::vector<float> limit(const std::vector<float> &command) { return limit(command[0], command[1]); }
...
//  unsigned short last_timestamp;
  ...
  float last_vx, last_wz; // In [m/s] and [rad/s]
  float command_vx, command_wz; // In [m/s] and [rad/s]
