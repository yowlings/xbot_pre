 ...
 *
 * @brief Flash ftdi chip's serial number with 'xbot_' prefix.
 *
 * Flash automatically when there is not flashed FTDI devices.
 * (Later)This program accept loop mode
 *
 * <b>License:</b> BSD https://raw.github.com/yujinrobot/xbot_core/hydro-devel/xbot_ftdi/LICENSE
 *
 **/

#include <iostream>

#include "../include/xbot_ftdi/scanner.hpp"
#include "../include/xbot_ftdi/writer.hpp"
...
int main(int argc, char** argv)
{
  ...
  for (unsigned int i=0; i<no_devices; i++) {
    ...
    //writing xbot as serial id
    if( serial_id.substr(0,6) == std::string("xbot") )
    {
      std::cout << "Already flashed as [" << serial_id << "]." << std::endl;
      //return 1; // if already xbot
      continue;
    }

    std::string new_id = "xbot_" + serial_id; // should be less than 20 character.
    ...
    if (ret_val < 0) {
      std::cout << ret_val << ": ";
      std::cout << "Something went wrong." << std::endl;
      return -1; // if failed to writing xbot
    }
  }
